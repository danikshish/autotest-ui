...
def initialize_browser_state(playwright: Playwright):
    ...
    context.storage_state(path='browser_state2.json')
    browser.close()

@pytest.fixture
def chromium_page_with_state(request: SubRequest, initialize_browser_state, playwright: Playwright) -> Generator[Page, Any, None]:
    yield from initialize_playwright_page(playwright, test_name=request.node.name, storage_state='browser_state2.json')
